...
export class OverviewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.average7Days = this.convertPercentage(this.getAverage7Days(), this.userItem.objective);
    this.average30Days = this.convertPercentage(this.getAverage30Days(), this.userItem.objective);
    this.daysGoalAchieved = this.getdaysGoalAchieved();
    this.objectiveProgressToday = !!this.getobjectiveProgressToday() ? this.getobjectiveProgressToday()/this.userItem.objective : 0 ;
  }
  ...
  public getAverage7Days(): number {
    if(this.userItem?.records.length >= 7) {
      ...
      return this.userItem?.records
        ...
        .slice(0, 6)
        .map(record => record.value)
        .reduce((valor1,valor2) => valor1 + valor2 ) [0] / this.userItem?.records.length;
    }
  }
  ...
  public getdaysGoalAchieved(): number {
    return this.userItem?.records.filter(record => record.value >= this.userItem.objective).length;
  }

  //progreso hoy
  public getobjectiveProgressToday(): number {
    return !!this.userItem?.records.find(record => new Date(record.date).toDateString() === new Date().toDateString()) ? this.userItem?.records.find(record => new Date(record.date).toDateString() ===  new Date().toDateString()).value : 0;
  }
